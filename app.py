import datetime
from exchangelib import Credentials, Account, EWSDateTime, EWSTimeZone, Q, Configuration
...
def sent_messages_count(account, start_date, end_date):
    # 1. Количество отправленных сообщений за период
    sent_folder = account.sent
    q = Q(datetime_received__range=(start_date, end_date))
    return sum(1 for _ in sent_folder.filter(q))


def received_messages_count(account, start_date, end_date):
    # 2. Количество получаемых сообщений за период
    inbox_folder = account.inbox
    q = Q(datetime_received__range=(start_date, end_date))
    return sum(1 for _ in inbox_folder.filter(q))
...
def replied_messages_count(account, received_messages):
    ...
    replied_msg_ids = [msg.in_reply_to for msg in account.sent.all()]
...
if __name__ == '__main__':
    ...
    config = Configuration(server="smtp.invitro.ru", max_connections=10)
